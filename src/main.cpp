/*
 * Lafvin / Waveshare ESP32-C6 + 1.47" SPI ST7789  (172×320)
 ...
 *     Row 1 – progress bar + %  (amber when paused, teal when running)
 *     Row 2 – current layer / total layers
 *     Row 3 – ETA in Copenhagen local time  (or remaining minutes if NTP
 *             hasn't synced yet)
 *   Browser page at the ESP's IP shows a simple status page.
 *
 * Wiring (Waveshare / Lafvin ESP32-C6-LCD-1.47)
 * ─────────────────────────────────────────────
 *  SPI CLK  → GPIO 7    SPI MOSI → GPIO 6
 *  LCD CS   → GPIO 14   LCD DC   → GPIO 15
 *  LCD RST  → GPIO 21   LCD BL   → GPIO 22
 */
...
static LGFX        display;
static LGFX_Sprite canvas(&display);  // off-screen back-buffer (allocated at boot)
...
static void drawPrintStatus() {
    ...
    gfx.setTextColor(C_DIM);
    gfx.drawString(
        (cfg_pip + "   WiFi: " + WiFi.localIP().toString()).c_str(), W / 2, 165);
...
static void mqttMaintain() {
    ...
    String clientId = "esp32c6-" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void setup() {
    ...
    display.init();
    display.setRotation(3);   // landscape 320 × 172
    display.setBrightness(220);
    display.fillScreen(TFT_BLACK);

    // Allocate off-screen sprite for flicker-free double-buffered rendering.
    // 320×172×2 bytes = ~110 KB – only works if enough contiguous heap is free.
    g_spriteOK = (canvas.createSprite(320, 172) != nullptr);
